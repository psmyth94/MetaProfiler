create_GraPhlAn_files <- function(Object, graphlan_dir, var, day, filename, output_dir = getwd(), ranks = get_ranks(Object@master_tbl), by = ranks[1], min_distinct_peptide = 2, min_cell_nb = 2) {
  value_tbl <- NULL
  ...
  data <- na.omit(copy(Object@master_tbl), unique(by, ranks[1]))
  if(by %like% "category") {
    data = data[get(by) != "Function unknown"]
  }
  lca_column = colnames(data)[tolower(colnames(data)) %like% "lca"]
  rank_column = colnames(data)[tolower(colnames(data)) %like% "rank"]
  for(r in ranks) {
    by2 <- c(r, by)
    cols <- c(lca_column, by, rank_column, "value", "p.value")
    data2 <- copy(data)
    data2 <- data2[, test := (.N >= min_distinct_peptide) & (length(na.omit(unlist(.SD))) >= 2), by = by2, .SDcols = get_cols(var, day)]
    data2 <- data2[(test), -"test"]
    value_tbl <- rbind(value_tbl, setnames(na.omit(data2[,.(r = r, t.test(na.omit(unlist(.SD)))$estimate, t.test(na.omit(unlist(.SD)))$p.value), by = by2,  .SDcols = get_cols(var, day)]), cols), fill = T)
  }
  ...
  all_lineage <- unique(rbindlist(apply(all_lineage, 1, function(x) {
    x <- zoo::na.trim(x)
    x[is.na(x)] <- ""
    res <- NULL
    for(i in 1:length(x)) {
  ...
  unique_lineage <- rbindlist(phylo_tmp)
  ...
  annot_back <- setnames(data.table(col[1:n], na.omit(unique(all_lineage[[by]]))), c("col", by))
  ...
  annot <- paste0(apply(all_lineage, 1, function(x) {
    ...
    to_get <- setnames(data.table(x[length(x)], x[by], names(x)[length(x)]), c(lca_column, by, rank_column))
    val <- value_tbl[to_get, value, on = on]
    paste0(
      paste(paste0(tolower(substring(names(x), 1, 1)), "__", gsub("\\W+", "_", x), collapse = "."),
            "clade_marker_size", val, sep = "\t"),
      "\n",
      paste(paste0(tolower(substring(names(x), 1, 1)), "__", gsub("\\W+", "_", x), collapse = "."),
  ...
  for(r in ranks) {
    ...
    nm$tmp <- ifelse(grepl(" ", nm[[r]]), paste0(tolower(substring(r,1,1)),
                                               substring(nm[[r]],1,1),
                                               stri_trim_both(stri_extract_first_regex(nm[[r]], " \\w"))), 
  ...
  tbl2 <- unique(na.omit(setnames(data.table(rnk, tax, b, id), c(rank_column, lca_column, by, "l"))))
  if(grepl("category", by)) {
    Letter_NOG_Category <- fread("Letter_NOG_Category.csv")
    cat <- unique(value_tbl[[by]])
    tbl2 <- rbind(setnames(data.table(by, cat, cat, Letter_NOG_Category[cat, Letter, on = "Name"]), c(rank_column, lca_column, by, "l")), tbl2)
  }
  
  annot <- paste(annot, paste0(apply(all_lineage, 1, function(x) {
    x <- x[!is.na(x)]
    t <- tbl2[setnames(data.table(x[length(x)], x[1], names(x)[length(x)]), c(lca_column, by, rank_column)), on = c(lca_column, by, rank_column)]
    if(!is.na(t$l)) {
      paste(paste0(tolower(substring(names(x), 1, 1)), "__", gsub("\\W+", "_", x), collapse = "."),
            "\tannotation\t", t$l, sep = "")
    } else {
  ...
  annot_lgnd <- Legend(at, title = "Annotation Legend:", legend_gp = gpar(fill = NA, fontsize = 12, fontfamily = "sans"), ncol = 4)
  annot_width = as.numeric(convertUnit(widthDetails(annot_lgnd@grob), "in")) + 0.08
  annot_height = as.numeric(convertUnit(heightDetails(annot_lgnd@grob), "in"))
  ...
  color_key = Legend(col_fun = col_fun, title = "Labeling Ratio (%)", title_position = "leftcenter-rot", grid_height = unit(10, "mm"))
  color_key_width = as.numeric(convertUnit(widthDetails(color_key@grob), "in")) + 0.08
  color_key_height = as.numeric(convertUnit(heightDetails(color_key@grob), "in")) + 0.08
  save_plot(filename = "LR_color_key.png", color_key, width = color_key_width, height = color_key_height)
  mp <- rasterGrob(png::readPNG("plot.png"))
  al <- rasterGrob(png::readPNG("annotation_legend.png"))
  ck <- rasterGrob(png::readPNG("LR_color_key.png"))
  
  if(grepl("category", by)) {
    cog_lgnd <- paste0(Letter_NOG_Category[annot_back[[by]], Letter, on = "Name"], ": ", annot_back[[by]])
    cog_lgnd <- Legend(cog_lgnd[order(nchar(cog_lgnd))], title = "COG Category Legend:", legend_gp = gpar(fill = annot_back$col[order(nchar(cog_lgnd))]), ncol = 3)
    cog_width = as.numeric(convertUnit(widthDetails(cog_lgnd@grob), "in")) + 0.1
    cog_height = as.numeric(convertUnit(heightDetails(cog_lgnd@grob), "in"))
    ...
    cl <- rasterGrob(png::readPNG("col2nog.png"))
    ag <- arrangeGrob(grobs = list(cl, mp, al, ck), layout_matrix = matrix(c(1,2,2,2,3,NA,4,NA,NA,NA), ncol = 2), heights = c(cog_height * annot_width/cog_width, rep(annot_width/3, 3), annot_height), widths = c(annot_width, color_key_width * (annot_width/3)/color_key_height))
    ...
    ag <- arrangeGrob(grobs = list(mp, al, ck), layout_matrix = matrix(c(1,1,1,2,3,NA,NA,NA), ncol = 2), heights = c(rep(annot_width/3, 3), annot_height), widths = c(annot_width, color_key_width * (annot_width/3)/color_key_height))
